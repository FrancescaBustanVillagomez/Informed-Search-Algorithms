...
MAPS = [
    os.path.join(os.path.dirname(BASE_DIR), "mappe", "lgt101d.map"), #Mappa piccola
    os.path.join(os.path.dirname(BASE_DIR), "mappe", "den203d.map"), #Mappa media
    os.path.join(os.path.dirname(BASE_DIR), "mappe", "den401d.map"), #Mappa grande
]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE and start and end and not algorithm_running and not finished:
                        ...
                        result = load_map(MAPS[current_map_idx])
                         # Carica mappa
                        #result = load_map(MAP)
